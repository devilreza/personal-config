...
# Neovim Installation Script for Go Development
...
print_header "Neovim Installation for Go Development"
...
# Install Neovim plugins (with timeout to prevent hanging)
print_info "Installing Neovim plugins..."
timeout 300 nvim --headless -c "autocmd User LazyComplete quitall" -c "Lazy sync" || {
    print_warning "Plugin installation timed out or failed, but continuing..."
    print_info "Plugins will be installed on first Neovim launch"
}

# Build telescope fzf extension
print_info "Building telescope fzf extension..."
if [ -d "$HOME/.local/share/nvim/lazy/telescope-fzf-native.nvim" ]; then
    cd "$HOME/.local/share/nvim/lazy/telescope-fzf-native.nvim"
    make clean || true
    make || true
    cd - > /dev/null
    print_success "Telescope fzf extension built"
else
    print_info "Telescope fzf extension will be built on first Neovim launch"
fi

print_success "Neovim plugins installation completed (or will complete on first launch)"

print_footer "Neovim installation completed!"
...
print_info "2. Explore the Go development features"
print_info "3. Use :GoInstallBinaries to install additional Go tools"
